...
<html>
  <head>
	...
	function upUser(){
		...
		var c_w=(s_w-w_w)/2;
		var c_h=(s_h-w_h)/2;
		window.open("upUser.html","","width="+w_w+"px,height="+w_h+"px,top="+c_h+"px,left="+c_w+"px"); 
	}
	function delUser(id){ 
  ...
  <div id="all">
    ...
    <c:forEach items="${requestScope.allUser }" var="user">
	    <tr id="t${user.id }">
		    ...
		    <td>${user.age }</td>
		    <td>${user.address }</td>
		    <td><a href="javascript:void(0)" onclick="upUser()">修改</a>&nbsp;<a href="javascript:void(0)" onclick="return delUser(${user.id })">删除</a></td>
	    </tr>
    </c:forEach>
    </table>
   </div>
